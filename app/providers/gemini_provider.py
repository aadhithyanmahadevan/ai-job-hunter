import json

from google import genai

from app.config.settings import settings


class GeminiProvider:

    def __init__(self):
        self.client = genai.Client(
            api_key=settings.GEMINI_API_KEY
        )

        self.models = [
            "models/gemini-3.5-flash",
            "models/gemini-2.5-flash",
            "models/gemini-2.5-flash-lite",
            "models/gemini-3.1-flash-lite",
            "models/gemini-2.0-flash",
        ]

        self.last_working_index = 0

    # ---------------------------------------------------------
    # Internal Generator
    # ---------------------------------------------------------

    def _generate(self, prompt: str):

        ordered_models = (
            self.models[self.last_working_index:]
            + self.models[: self.last_working_index]
        )

        last_error = None

        for model in ordered_models:

            try:

                response = self.client.models.generate_content(
                    model=model,
                    contents=prompt,
                )

                self.last_working_index = self.models.index(model)

                return response.text

            except Exception as e:

                last_error = e

        raise RuntimeError(
            f"All Gemini models failed: {last_error}"
        )

    # ---------------------------------------------------------
    # Helper
    # ---------------------------------------------------------

    def ask(self, prompt: str):

        return (
            self._generate(prompt)
            .replace("```json", "")
            .replace("```", "")
            .strip()
        )

    # ---------------------------------------------------------
    # Resume Analysis
    # ---------------------------------------------------------

    def analyze_resume(self, resume_text: str):

        prompt = f"""
You are an expert recruiter.

Analyze the resume.

Return ONLY valid JSON.

{{
    "name":"",
    "title":"",
    "years_of_experience":"",
    "ats_score":0,
    "skills":[],
    "education":[],
    "certifications":[],
    "projects":[],
    "strengths":[],
    "missing_skills":[],
    "suggestions":[]
}}

Resume:

{resume_text}
"""

        response = self.ask(prompt)

        try:

            return json.loads(response)

        except Exception:

            return {
                "raw_response": response
            }

    # ---------------------------------------------------------
    # Extract Job Skills
    # ---------------------------------------------------------

    def extract_job_skills(self, description: str):

        prompt = f"""
Extract ONLY technical skills.

Return ONLY JSON.

{{
    "skills":[]
}}

Job Description:

{description}
"""

        try:

            return json.loads(
                self.ask(prompt)
            )

        except Exception:

            return {
                "skills": []
            }

    # ---------------------------------------------------------
    # Interview Questions
    # ---------------------------------------------------------

    def generate_interview_questions(
        self,
        job_description: str,
    ):

        prompt = f"""
Return ONLY JSON.

{{
    "technical":[],
    "hr":[],
    "coding":[]
}}

Job Description:

{job_description}
"""

        try:

            return json.loads(
                self.ask(prompt)
            )

        except Exception:

            return {
                "technical": [],
                "hr": [],
                "coding": [],
            }

    # ---------------------------------------------------------
    # Resume Improvements
    # ---------------------------------------------------------

    def resume_improvements(
        self,
        resume_text: str,
        job_description: str,
    ):

        prompt = f"""
Return ONLY JSON.

{{
    "strengths":[],
    "weaknesses":[],
    "suggestions":[]
}}

Resume:

{resume_text}

Job Description:

{job_description}
"""

        try:

            return json.loads(
                self.ask(prompt)
            )

        except Exception:

            return {
                "strengths": [],
                "weaknesses": [],
                "suggestions": [],
            }

    # ---------------------------------------------------------
    # Career Recommendation
    # ---------------------------------------------------------

    def career_recommendation(
        self,
        resume: dict,
    ):

        prompt = f"""
Return ONLY JSON.

{{
    "summary":"",
    "next_steps":[]
}}

Resume:

{json.dumps(resume)}
"""

        try:

            return json.loads(
                self.ask(prompt)
            )

        except Exception:

            return {
                "summary": "",
                "next_steps": [],
            }

    # ---------------------------------------------------------
    # Learning Roadmap
    # ---------------------------------------------------------

    def learning_roadmap(
        self,
        missing_skills: list,
    ):

        prompt = f"""
Return ONLY JSON.

{{
    "weeks":[]
}}

Missing Skills:

{json.dumps(missing_skills)}
"""

        try:

            return json.loads(
                self.ask(prompt)
            )

        except Exception:

            return {
                "weeks": [],
            }

    # ---------------------------------------------------------
    # Resume ↔ Job Matching
    # ---------------------------------------------------------

    def match_resume_to_job(
        self,
        resume_text: str,
        job_description: str,
    ):

        prompt = f"""
You are a Senior Technical Recruiter, ATS Expert, and Software Engineering Hiring Manager.

Compare the candidate's resume against the job description.

Evaluate:

1. Technical Skills
2. Programming Languages
3. Frameworks
4. Testing Tools
5. Cloud Technologies
6. Databases
7. CI/CD Knowledge
8. Experience
9. ATS Keyword Match
10. Overall Suitability

Resume:

{resume_text}

------------------------------------------------------------

Job Description:

{job_description}

------------------------------------------------------------

Return ONLY valid JSON.

Use EXACTLY this schema.

{{
    "score": 92,
    "strengths": [
        "Java",
        "Spring Boot",
        "Selenium",
        "REST Assured"
    ],
    "missing_skills": [
        "Playwright",
        "Docker",
        "Azure DevOps"
    ],
    "matched_keywords": [
        "Java",
        "Automation",
        "REST API"
    ],
    "experience_match": "Excellent",
    "recommendation": "Strong candidate. Learning Playwright and Azure DevOps would significantly improve the profile.",
    "reasoning": "The resume closely aligns with the job requirements and demonstrates strong automation testing experience."
}}

Rules:

- Return ONLY JSON.
- No markdown.
- No explanation.
- score must be between 0 and 100.
- strengths must contain only skills.
- missing_skills must contain only missing technical skills.
- matched_keywords must contain ATS keywords found in both resume and job description.
- experience_match must be one of:
  "Excellent", "Good", "Average", "Weak".
- recommendation should be short.
- reasoning should be 1 or 2 sentences.
"""

        response = self.ask(prompt)

        try:

            result = json.loads(response)

            return {
                "score": result.get("score", 0),
                "strengths": result.get("strengths", []),
                "missing_skills": result.get("missing_skills", []),
                "matched_keywords": result.get("matched_keywords", []),
                "experience_match": result.get("experience_match", "Unknown"),
                "recommendation": result.get("recommendation", ""),
                "reasoning": result.get("reasoning", ""),
            }

        except Exception:

            return {
                "score": 0,
                "strengths": [],
                "missing_skills": [],
                "matched_keywords": [],
                "experience_match": "Unknown",
                "recommendation": "Unable to analyze the resume.",
                "reasoning": "",
                "raw_response": response,
            }